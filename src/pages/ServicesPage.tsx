...
function IndexCard({ service: s, featured, visible, delay }: { service: ServiceDef; featured?: boolean; visible: boolean; delay: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="flex-1 text-[13.5px] leading-relaxed text-[var(--color-text-secondary)]" style={{ fontFamily: 'var(--font-body)' }}>{s.valueProp}</p>
      <span className="link-arrow text-[13px] font-semibold" style={{ color: s.color, fontFamily: 'var(--font-body)' }}>
        Learn more
        <svg width="13" height="13" viewBox="0 0 12 12" fill="none"><path d="M2.5 6h7M6 2.5L9.5 6 6 9.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" /></svg>
      </span>
    </Link>
  )
}
