...
const PRODUCTS: (Opt & { weight: number })[] = [
  { id: 'packaging', weight: 0.08, f: 0, label: { en: 'Packaging / container', ar: 'تغليف / عبوة' } },
  { id: 'electronics', weight: 0.35, f: 0, label: { en: 'Consumer electronics', ar: 'إلكترونيات استهلاكية' } },
  { id: 'apparel', weight: 0.5, f: 0, label: { en: 'Apparel / textile item', ar: 'ملابس / منسوجات' } },
  { id: 'appliance', weight: 12, f: 0, label: { en: 'Household appliance', ar: 'جهاز منزلي' } },
  { id: 'furniture', weight: 18, f: 0, label: { en: 'Furniture item', ar: 'قطعة أثاث' } },
  { id: 'auto', weight: 6, f: 0, label: { en: 'Automotive component', ar: 'مكوّن سيارات' } },
  { id: 'construction', weight: 25, f: 0, label: { en: 'Construction element', ar: 'عنصر بناء' } },
  { id: 'food', weight: 0.45, f: 0, label: { en: 'Packaged food & beverage', ar: 'أغذية ومشروبات مغلّفة' } },
]

// ── Raw materials → cradle-to-gate kg CO₂e/kg (ICE v3 / ecoinvent) ────────
const MATERIALS: Opt[] = [
  { id: 'alu_v', f: 16.1, recycledTo: 'alu_r', label: { en: 'Aluminium — virgin', ar: 'ألمنيوم — بكر' } },
  { id: 'alu_r', f: 1.6, label: { en: 'Aluminium — recycled', ar: 'ألمنيوم — معاد تدويره' } },
  { id: 'steel_v', f: 2.5, recycledTo: 'steel_r', label: { en: 'Steel — virgin (BF-BOF)', ar: 'صلب — بكر (BF-BOF)' } },
  { id: 'steel_r', f: 0.65, label: { en: 'Steel — recycled (EAF)', ar: 'صلب — معاد تدويره (EAF)' } },
  { id: 'stainless', f: 6.15, label: { en: 'Stainless steel', ar: 'صلب مقاوم للصدأ' } },
  { id: 'plastic_v', f: 3.1, recycledTo: 'plastic_r', label: { en: 'Plastic — virgin', ar: 'بلاستيك — بكر' } },
  { id: 'plastic_r', f: 1.1, label: { en: 'Plastic — recycled', ar: 'بلاستيك — معاد تدويره' } },
  { id: 'glass', f: 0.9, label: { en: 'Glass', ar: 'زجاج' } },
  { id: 'paper', f: 0.95, label: { en: 'Paper / cardboard', ar: 'ورق / كرتون' } },
  { id: 'cotton', f: 5.9, label: { en: 'Cotton / natural fibre', ar: 'قطن / ألياف طبيعية' } },
  { id: 'polyester', f: 3.4, label: { en: 'Synthetic textile (polyester)', ar: 'نسيج صناعي (بوليستر)' } },
  { id: 'wood', f: 0.35, label: { en: 'Wood / timber', ar: 'خشب' } },
  { id: 'concrete', f: 0.15, label: { en: 'Concrete / mineral', ar: 'خرسانة / معدني' } },
  { id: 'electronics', f: 25, label: { en: 'Electronics (mixed + PCB)', ar: 'إلكترونيات (مختلطة + لوحات)' } },
]

// ── Manufacturing process → added kg CO₂e/kg ─────────────────────────────
const PROCESSES: Opt[] = [
  { id: 'assembly', f: 0.3, label: { en: 'Manual / low-energy assembly', ar: 'تجميع يدوي / منخفض الطاقة' } },
  { id: 'moulding', f: 0.9, label: { en: 'Injection moulding', ar: 'قولبة بالحقن' } },
  { id: 'stamping', f: 0.6, label: { en: 'Metal stamping / forming', ar: 'ختم / تشكيل معدني' } },
  { id: 'machining', f: 1.8, label: { en: 'Machining (CNC)', ar: 'تصنيع آلي (CNC)' } },
  { id: 'casting', f: 1.3, label: { en: 'Casting / forging', ar: 'صبّ / طرق' } },
  { id: 'firing', f: 2.6, label: { en: 'Kiln / high-heat firing', ar: 'فرن / حرق عالي الحرارة' } },
  { id: 'weaving', f: 1.5, label: { en: 'Weaving / textile finishing', ar: 'نسيج / تشطيب منسوجات' } },
  { id: 'assembly_e', f: 3.2, label: { en: 'Electronics assembly', ar: 'تجميع إلكترونيات' } },
]

// ── Packaging → kg CO₂e per kg of product ────────────────────────────────
const PACKAGING: Opt[] = [
  { id: 'minimal', f: 0.02, label: { en: 'None / minimal', ar: 'لا شيء / أدنى حد' } },
  { id: 'cardboard_r', f: 0.05, label: { en: 'Recycled cardboard', ar: 'كرتون معاد تدويره' } },
  { id: 'cardboard_v', f: 0.12, label: { en: 'Cardboard + some plastic', ar: 'كرتون + بعض البلاستيك' } },
  { id: 'plastic', f: 0.25, label: { en: 'Plastic-heavy / blister', ar: 'بلاستيك كثيف / بليستر' } },
  { id: 'composite', f: 0.3, label: { en: 'Mixed / composite (hard to recycle)', ar: 'مختلط / مركّب (صعب التدوير)' } },
]

// ── Transport method → kg CO₂e per tonne-km (DEFRA 2023) ─────────────────
const TRANSPORT: Opt[] = [
  { id: 'sea', f: 0.011, label: { en: 'Sea freight', ar: 'شحن بحري' } },
  { id: 'rail', f: 0.027, label: { en: 'Rail freight', ar: 'شحن بالسكك' } },
  { id: 'road', f: 0.107, label: { en: 'Road (truck)', ar: 'طريق (شاحنة)' } },
  { id: 'air', f: 0.8, label: { en: 'Air freight', ar: 'شحن جوي' } },
]

const DISTANCES: (Opt & { km: number })[] = [
  { id: 'local', km: 200, f: 0, label: { en: 'Local (~200 km)', ar: 'محلي (~200 كم)' } },
  { id: 'regional', km: 1500, f: 0, label: { en: 'Regional (~1 500 km)', ar: 'إقليمي (~1500 كم)' } },
  { id: 'intercont', km: 8000, f: 0, label: { en: 'Intercontinental (~8 000 km)', ar: 'عابر للقارات (~8000 كم)' } },
]

type RatingKey = 'A' | 'B' | 'C' | 'D' | 'E'
const RATING: Record<RatingKey, { color: string; label: Bi }> = {
  A: { color: '#16A34A', label: { en: 'Excellent', ar: 'ممتاز' } },
  B: { color: 'var(--color-lime)', label: { en: 'Good', ar: 'جيد' } },
  C: { color: '#E8850C', label: { en: 'Moderate', ar: 'متوسط' } },
  D: { color: '#F97316', label: { en: 'High impact', ar: 'أثر مرتفع' } },
  E: { color: '#DC2626', label: { en: 'Very high impact', ar: 'أثر مرتفع جدًا' } },
}
...
export default function LcaQuickScan() {
  ...
  const insights: Bi[] = useMemo(() => {
    ...
    if (result.tr.id === 'air') {
      ...
      out.push({ en: 'Powering manufacturing with renewable electricity is the fastest lever to lower this stage.', ar: 'تشغيل التصنيع بالكهرباء المتجددة هو أسرع رافعة لخفض هذه المرحلة.' })
    } else if (hp === 'packaging') {
      out.push({ en: 'Move to recyclable mono-material packaging to reduce this stage.', ar: 'انتقل إلى تغليف أحادي المادة قابل للتدوير لخفض هذه المرحلة.' })
    }
