...
function DashboardMock({ color }: { color: string }) {
  const rows = [
    { label: 'Mesaieed Facility', val: 92, status: 'Optimal' },
    { label: 'Ras Laffan LNG', val: 88, status: 'Optimal' },
    { label: 'Ruwais Cement', val: 74, status: 'Review' },
  ]
  return (
  ...
  return (
    <div className="bg-[var(--color-surface)] rounded-2xl p-5 shadow-lg">
      ...
      <div className="flex justify-between mt-2 text-[9px] text-[var(--color-text-secondary)]" style={{ fontFamily: 'var(--font-body)' }}>
        ...
        <span style={{ color }}>May</span><span style={{ color }}>Jun</span>
        <span style={{ color }}>Jul</span><span style={{ color }}>Aug</span>
      </div>
    </div>
  )
}
...
function ReportMock({ color }: { color: string }) {
  const items = ['CBAM Declaration', 'IFRS S2 Disclosure', 'Qatar NCAP Report', 'Third-Party Verification']
  return (
    <div className="bg-[var(--color-surface)] rounded-2xl p-5 shadow-lg">
      <div className="text-[12px] font-semibold text-[var(--color-text-primary)] mb-4" style={{ fontFamily: 'var(--font-body)' }}>
        Compliance Reports — Q3 2025
      </div>
      {items.map((item, i) => (
