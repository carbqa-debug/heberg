...
const SECTORS = [
  { label: 'LNG & Petrochemicals', factor: 0.38 },
  { label: 'Cement',               factor: 0.82 },
  { label: 'Steel',                factor: 0.54 },
  { label: 'Aluminium',            factor: 1.1  },
  { label: 'Fertilisers',          factor: 0.44 },
  { label: 'Hydrogen',             factor: 1.8  },
  { label: 'Electricity',          factor: 0.65 },
]
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        <div className={`text-center mb-16 fade-up ${visible ? 'visible' : ''}`}>
          ...
          <h2 className="text-[44px] lg:text-[56px] leading-[1.06] text-[var(--color-text-primary)]" style={{ fontFamily: 'var(--font-display)', letterSpacing: '-0.02em' }}>
            What's Your{' '}
            <span className="text-[var(--color-primary)]">CBAM Exposure?</span>
          </h2>
          <p className="mt-4 text-[16px] text-[var(--color-text-secondary)] max-w-[540px] mx-auto" style={{ fontFamily: 'var(--font-body)' }}>
            Estimate your annual EU Carbon Border Adjustment Mechanism cost based on your sector and export tonnage.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-1.5 text-[11px] text-[var(--color-text-secondary)]" style={{ fontFamily: 'var(--font-body)' }}>
                  Carbon intensity factor: {SECTORS[sector].factor} tCO₂/tonne product
                </p>
              </div>

              <div>
                <label className="text-[12px] font-semibold text-[var(--color-text-secondary)] uppercase tracking-wide mb-2 block" style={{ fontFamily: 'var(--font-body)' }}>
                  Annual EU Export Volume (thousand tonnes)
                </label>
                <input
                  type="number"
                  min="1"
              ...
              <div className="bg-[var(--color-bg-secondary)] rounded-xl p-4">
                <p className="text-[12px] text-[var(--color-text-secondary)] leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>
                  <strong className="text-[var(--color-text-primary)]">Assumptions:</strong> EUA spot price €{CBAM_PRICE}/tCO₂ (indicative 2026). No verified emission data credited — uses default CBAM intensity values. Actual liability may vary.
                </p>
              </div>
            ...
            {!result ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                ...
                <p className="text-[16px] text-[var(--color-text-secondary)]" style={{ fontFamily: 'var(--font-body)' }}>
                  Enter your sector and export tonnage to see your estimated CBAM liability
                </p>
              </div>
            ) : (
              <>
                <div>
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    >
                      {riskLevel === 'high' ? 'HIGH EXPOSURE' : riskLevel === 'medium' ? 'MEDIUM EXPOSURE' : 'MANAGEABLE EXPOSURE'}
                    </span>
                  </div>

                  <p className="text-[13px] text-[var(--color-text-secondary)] mb-2" style={{ fontFamily: 'var(--font-body)' }}>Estimated annual CBAM liability</p>
                  <div className="text-[52px] lg:text-[64px] font-bold leading-none mb-1" style={{ fontFamily: 'var(--font-display)', color: riskLevel === 'high' ? 'var(--color-danger)' : riskLevel === 'medium' ? 'var(--color-warning)' : 'var(--color-primary)' }}>
                    €{(low / 1000).toFixed(0)}K
                  </div>
                  <div className="text-[22px] text-[var(--color-text-secondary)] mb-4" style={{ fontFamily: 'var(--font-display)' }}>
                    — €{(high / 1000).toFixed(0)}K / year
                  </div>
                  ...
                  <div className="bg-[color-mix(in_srgb,var(--color-surface)_60%,transparent)] rounded-xl p-4 mb-4">
                    <p className="text-[12px] text-[var(--color-text-secondary)]" style={{ fontFamily: 'var(--font-body)' }}>
                      Implied carbon mass: <strong className="text-[var(--color-text-primary)]">{(result.intensity / 1000).toFixed(1)}k tonnes CO₂</strong><br />
                      At verified emission intensity, liability could reduce by up to <strong className="text-[var(--color-primary)]">40–65%</strong>
                    </p>
                  </div>

                  <p className="text-[10px] text-[var(--color-neutral-400)] italic" style={{ fontFamily: 'var(--font-body)' }}>
                    Indicative estimate only. Not financial advice. Subject to EUA price and actual verified intensity.
                  </p>
                </div>
